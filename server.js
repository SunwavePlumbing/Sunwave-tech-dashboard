...
async function fetchTechEmployees() {
  ...
  const enforceActiveFilter = activeTechIds && activeTechIds.size > 0;
  if (!enforceActiveFilter) {
    console.warn('[employees] active-tech-ids empty; falling back to employee role/name list');
  }
  const all = [];
  let page = 1;
  while (page <= 10) {
